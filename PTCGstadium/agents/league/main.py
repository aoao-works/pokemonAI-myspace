"""リーグ学習モデル (ptcg_league_model.pth) を使うエージェントラッパー。"""
...
_spec = importlib.util.spec_from_file_location('_sub_league', os.path.join(SUBMISSION, 'main.py'))
_sub  = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_sub)

# ptcg_league_model.pth で強制上書き
...
_net = _Net()
_net.load_state_dict(torch.load(os.path.join(SUBMISSION, 'ptcg_league_model.pth'), map_location='cpu', weights_only=True))
_net.eval()
_sub._model = _net
...
print("[league agent] loaded ptcg_league_model.pth")
