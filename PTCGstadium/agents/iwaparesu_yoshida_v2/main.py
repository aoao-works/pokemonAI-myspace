...
# --- ポケモンID（イワパレスデッキ）---
# イシズマイ→イワパレスの進化ラインに加え、マリガン率低減と初手の壁役を兼ねる
# 独立たねポケモン・ザルードを採用（v3での実測: たね4枚のみだと初手マリガン率
# 60.1%、8枚まで増やすと34.6%に低減）。
CID_ISHIZUMAI = 344   # イシズマイ（たね, HP70, にげ2, かくせい=自己進化 → イワパレス）
CID_IWAPARESU = 345   # イワパレス（1進化, HP150, グレートシザー 草無無 120）★主力
                      # 特性しんぴのいしやど（ex のワザダメージを受けない）は受動＝発動不要
CID_ZARUDE    = 178   # ザルード（たね, HP120, にげ1, リーフドレイン=草1で20+自己回復20）
                      # かくせいは「攻撃」なのでイシズマイはバトル場でしか使えない＝
                      # 最初からイシズマイを場に出すと無防備な状態で相手の初撃に晒される。
                      # ザルードを初手アクティブにして被弾を受け止めつつ、ベンチのイシズマイに
                      # 先にエネルギーを付けておき、機が来たら退場→交代即かくせいで
                      # 無防備な期間をほぼゼロにする狙い（_SETUP_ACTIVE_PRIORITY 参照）。
...
CID_POTION      = 1112  # いいきずぐすり（60回復＋エネ1個トラッシュ＝諸刃, デッキには不採用）
CID_JUMBO_ICE   = 1147  # ジャンボアイス（エネ3個以上のバトルを80回復。ザルード採用時に4→2枚へ減量）
CID_BELL        = 1190  # ベルのまごころ（残HP30以下の味方を全回復, サポート）

# --- 退避用どうぐ ---
CID_EMERGENCY_BOARD = 1157  # 緊急ボード（にげるエネ-1、残HP30以下なら無料でにげられる）

# --- 定番トレーナー（複数デッキ共通。無いデッキは 0 で無効化）---
CID_BOSS            = 1182  # ボスの指令
CID_LILLIE          = 1227  # リーリエの決心
CID_MATSUBA         = 1187  # マツバの確信（手札1枚トラッシュ→相手ベンチ数ぶんドロー）
                            # 2026-08-01: デッキ内0枚化（クセロシキ3枚目の枠確保のため削減）。
                            # コード上は無効化スイッチ(CID_POKE_PAD等)と同じ扱いで残置。
CID_POKE_PAD        = 0     # このデッキには無い（弱点の炎対策としてクリスピン/リーリアのアピールの枠を優先し不採用）
CID_ULTRA_BALL      = 0     # このデッキには無い
CID_NIGHT_STRETCHER = 1097  # 夜のタンカ（トラッシュのポケモン/基本エネルギーを1枚手札へ）
                            # 実戦リプレイ解析（2026-07-31）: 6敗すべてで「サイドは互角以上
...
# 通常のターン1エネルギー付与とは別枠の追加アタッチなので、実質1ターンに2枚
# 貼れる＝主砲（グレートシザー、草無無=3エネ）の攻撃開始を1ターン早められる。
CID_CRISPIN = 1198
# リーリアのアピール: ボスの指令と同じ「相手ベンチを引きずり出す」効果＋混乱付与
# （たねポケモン限定）。ボスは4枚上限なので、実質的なボスをもう2枚増やせる。
CID_LISIA   = 1204

# --- スタジアム（団のアジト）---
# 「そのターン中に『ロケット団』名のサポート（ロケット団のラムダ）を使っていたら
# 2枚ドロー」。ラムダ(4枚)と組み合わせると実質サーチ+ドロー2の圧縮率で回るので、
# ドロー枯渇に悩みやすい壁デッキの生命線。優先的に場に出し、捨て先選択でも保護する。
CID_STADIUM = 1257
...
# ザルードは初手の壁・被弾役として1体いれば十分なので目標は1。
...
_FIELD_CAP: dict[int, int] = {
    CID_ISHIZUMAI: 5, CID_IWAPARESU: 5, CID_ZARUDE: 4,
}

# --- ベンチ配置の優先順（(カードID, すでに場にいる枚数)。小さいほど先に出す）---
# イシズマイを優先的にベンチへ（早めにエネルギーを付けて温存させるため）、
# ザルードはその後（基本は初手アクティブ運用がメインで、複数体は不要）。
...
_SEARCH_PRIORITY: dict[int, int] = {
    CID_ISHIZUMAI: 0, CID_ZARUDE: 1, CID_IWAPARESU: 2,
}

# --- エネルギーアタッチ優先度（小さいほど優先）。主攻撃役に集める ---
# ザルードはイワパレスの次点で、リーフドレイン（草1個で20+自己回復20）をすぐ使える
# ようにする。イシズマイは1個で かくせい が撃てるので優先度は最後でよい。
...
# ヒーローマント(+100HP) はイワパレスに優先して貼る。ザルードはHPが厚く緊急度が
# 下がるので中間。
...
# --- どうぐ固有の貼り先優先（カードIDごとの上書き。無指定は _TOOL_TARGET_PRIORITY を使う）---
# 緊急ボードは「進化前で狙われやすく退避手段も乏しいイシズマイの延命」が目的なので、
# 主砲強化系（ヒーローマント等）とは逆にイシズマイへ優先して貼る。
_TOOL_TARGET_PRIORITY_OVERRIDE: dict[int, dict[int, int]] = {
    CID_EMERGENCY_BOARD: {CID_ISHIZUMAI: 0, CID_ZARUDE: 1, CID_IWAPARESU: 2},
}
...
# ザルードを最優先。理由: イシズマイの「かくせい」は攻撃技なのでバトル場でしか
# 使えない＝最初からイシズマイを出すと無防備なままエネルギー0で相手の初撃に
# 晒される。ザルード（HP120, 自己回復技持ち）を身代わりにして受け止めつつ、
# ベンチのイシズマイへ先にエネルギーを付けておき、機を見て交代→即かくせいで
# 進化させれば、イシズマイが無防備なアクティブでいる期間をほぼゼロにできる。
...
# --- 絶対に捨てないカード（捨て先選択で保護）---
# 団のアジトはラムダとの2枚ドローエンジンの要（2枚しか無い）なので温存する。
_DISCARD_PROTECT: frozenset[int] = frozenset({CID_STADIUM})
...
_SUPPORTER_PRIORITY: dict[int, int] = {
    ...
    1225: 20,           # トウコ（進化ポケモン＋エネをサーチ）。2026-08-02: 0枚化（下記参照）。
                        # deck.csvにデータ駆動なので実害無し、CID_MATSUBAと同じ「inert sentinel」パターン。
...
# 2026-08-02: 3→4枚に増量（トウコ1→0枚で捻出）。2026-08-01のログでは「3枚は
# 引ければ正しく機能する、問題は弾数でなく引き運」と結論していたが、48試合の
# 集計解析でアラカザム系統の勝率が1勝7敗(12.5%)とまだ悪いままだったため、
# 「引き運対策＝初手までに1枚でも引ける確率を上げる」目的で4枚目を追加した
# （枚数を増やすこと自体が引き運対策の直接的なレバーであるため、過去の結論と
# 矛盾しない）。次回リプレイでアラカザム戦の勝率変化を要確認。
_XEROSIC_HAND_THRESHOLD = 7

# --- ポケモンをサーチするグッズ（盤面が埋まっていれば温存）---
# ポケパッド。なかよしポフィン(1086)は2026-07-31実戦リプレイ解析で「敗北の6/17が
# イシズマイを一度も場に出せないまま負け」というパターンが判明したため再採用した
# （HP70以下のたねポケモンをベンチに2枚までサーチでき、イシズマイ(HP70)がぴったり
# 対象。ザルード(HP120)は対象外なので実質イシズマイ専用サーチとして機能する）。
# ポケギア3.0(1122)はサポーターサーチなので含めない（常時有用）。
# 2026-08-01: 実戦リプレイ解析(55151278)で「ベンチが一度も埋まらないまま負け」た
# 敗北6件中5件がポフィンを一度も引けていなかったため2→3枚に増量（トウコ2→1枚で捻出）。
CID_POFFIN = 1086  # なかよしポフィン（HP70以下のたねポケモンを2匹までベンチへ）
...
def _lisia_priority(obs: Observation) -> int | None:
    """リーリアのアピールの優先度（ボスと同じ判断だが、たねポケモンにしか撃てない
    制約があるので対象をたね限定でフィルタする。優先度はボスよりわずかに低い
    6/56 にして、両方使える場面では制約の無いボスを先に使わせる）。"""
    state = obs.current
    if state is None:
        return 6
    your_idx   = state.yourIndex
    ...
    opp_active  = opp_state.active[0] if opp_state.active else None
    if your_active is None or len(your_active.energies) == 0:
        return 56 if _is_stalled(state) else None

    best_dmg = _best_attack_damage(your_active, card_cache, attack_cache)
    bench_ko = [
        p for p in opp_state.bench
        if p.hp <= best_dmg and (card_cache.get(p.id) is not None and card_cache[p.id].basic)
    ]
    if opp_active is not None and opp_active.hp <= best_dmg:
        return 6 if any(_is_ex_pokemon(p, card_cache) for p in bench_ko) else None
    if bench_ko:
        return 6
    return 56 if _is_stalled(state) else None
...
def _supporter_sort_key(card_id: int, obs: Observation) -> int:
    ...
    ボス/リーリエ/ベル/マツバ/リーリアのアピールは中核で個別判断、
    その他は DECK CONFIG の _SUPPORTER_PRIORITY。"""
    if card_id == CID_BOSS:
        prio = _boss_priority(obs)
        return prio if prio is not None else 999
    if card_id == CID_LISIA:
        prio = _lisia_priority(obs)
        return prio if prio is not None else 999
...
def _should_play_stadium(card_id: int, obs: Observation) -> bool:
    ...
    （引いた分を無駄打ちせず、ラムダとのドローエンジンを切らさず保護する）。"""
    state = obs.current
    if state is None:
        return True
...
def _best_play_index(matches: list[int], options, obs: Observation) -> int | None:
    ...
    スタジアム（団のアジト）はラムダとの2枚ドローの起点で、場に出すこと自体に
    テンポコストが無い（他の枠を圧迫しない）ため、供給を切らさないよう早めに設置する。"""
    card_cache = _get_card_data_cache()
