...
- **Alakazam-line matchup — still bad, still small-sample, root cause now
  ...
  **run #9追記（2026-08-02、インタラクティブ実行）**: ユーザー依頼で48試合分
  ...
  進むのが次の一手）。
  **run #10追記（2026-08-02）**: `55165652`（クセロシキ4枚化後の提出）の42試合を
  解析。アラカザム系統は6戦1勝5敗（16.7%）——4枚化前の集計（12.5%）から明確な
  改善は確認できず（母数6と小さいため断定はできない）。エンジンの`Result.reason`
  で正確に再分類したところ、負け全体の60%（12/20）がreason=3（場にポケモンなし）
  で、run #7/8時点の50%からむしろ比率が増加。カードデータを確認し、**アラカザムは
  HP140、こちらの最大打点（イワパレスのグレートシザー）は120で、進化後は理論上
  一撃で狩れない**ことを新たに確認（アブラHP50/ケーシィHP80は既存のボスの指令
  ロジックの射程内）。前回見送った「Pokégear/Toko経由でクセロシキを能動サーチ」
  案を今回実装（下記「Fix applied」参照）。**次回、この変更でアラカザム戦の
  勝率・クセロシキの実戦での使用タイミングが改善したか要確認**。
...
- **"No active Pokémon" (engine reason=3) losses remain the single largest loss
  ...
  **run #8追記（2026-08-01）**: この「冗長性を増やす」案を検証し、有効なレバーでは
  ないと判明・クローズ済み。詳細はResolvedセクションの該当項目を参照。
...
- **「クラスタル系統(イシズマイ/イワパレス)を倒された後に戻す冗長性を増やせないか」— run #7の
  forward noteで提案された案を検証、有効なレバーではないと判明（run #8, 2026-08-01）**:
  ref `55157226`の全36公開エピソードを、リプレイJSON内の完全非マスクの生ログ
  (`steps[0][0]['visualize']` + エンジン自身の`{reason, result, type:'Result'}`イベント)
  を使って再解析（下記run #8の「手法メモ」参照、この手法で従来の解析手法の誤分類も1件
  発見した）。reason=3（場にポケモンなし）の8敗のうち、**5/8は試合中に該当ライン
  (id 344/345)を2枚以上見ていた**（手札・ベンチ・トラッシュのいずれかに存在した実績
  あり）のに敗北しており、「予備が足りなかった」わけではなく場に出した後に倒されている
  （既知の炎弱点・アラカザム・非ex大打撃）。しかも該当カードは既に**各4枚（上限）**
  採用済みで、そもそも増枠の余地がない。残り3/8（該当ラインを1枚以下しか見ていない）を
  個別に追跡したところ、いずれも「ザルードが単騎のまま試合が進み、ダウェブルを一度も
  引けなかった」という既に把握済みのポフィン残存パターンと同一で、うち2件
  (`89292882`, `89300525`)は**全く同じ攻撃**——マツリーカ(Marnie's Grimmsnarl ex,
  id 648, attackId 937)の技で正確に-180ダメージ、ザルードを一撃で倒すもの——による
  決着だった（ザルードはイワパレス系統だけが持つex無効特性を持たないため無防備、想定
  通りの挙動）。**結論**: 増枠は不可能かつ根拠薄弱なためこのレバーはクローズ。新たな
  有効な改善案には至らなかった。
...
## 2026-08-02 (loop run #10, インタラクティブ実行 / iwaparesu-loop skill)

**オリエンテーション**: バックログ・run #9のログを確認。トップ項目はrun #9の
「クセロシキ4枚化後、アラカザム戦の勝率が改善したか要確認」——これが今回の
主な作業。

**現在の順位**: `55165652`（run #9のクセロシキ4枚化）が**616.5**（過去最高
スコア）でCOMPLETE。リーダーボード（フレッシュCSV、6116チーム）: 銅メダル
ボーダー（上位10%、611位）=**835.3**。現在の順位は**3328位**（過去最高
スコア616.5）——run #8時点の順位（3828位、スコア576.7）から改善。

**リプレイ解析（`55165652`の全42公開エピソード）**:
- 戦績: **22勝20敗（52.4%）**。前回48試合集計時の50%からわずかに改善。
- **手法メモ**: 過去のrun（本skillの前身、8/2の会話内サブエージェント解析）
  が使っていた「`prize`配列内のnull数」を「取った/取られた枚数」とみなす
  ヒューリスティックは、実際には配列長そのもの（＝残りプライズ枚数、
  全要素が伏せ札でnullなのは常に成立するため無意味な指標だった）を数えていた
  だけと判明し、誤解を招く出力だった。run #7で確立された**エンジン自身の
  `{reason, result, type:'Result'}`ログイベント**（`steps[N][*]['visualize'][*]
  ['logs']`または`observation.logs`内、どちらに現れるかは試合による）を
  唯一の正とする方針に統一——今後のrunでもプライズ配列のnullカウントは
  信頼できる指標として使わないこと。
- reason別内訳: **reason=3（場にポケモンなし）12/20（60%）**、reason=1
  （通常のサイド負け）5/20（25%）、reason=2（自分のデッキアウト）3/20
  （15%）。reason=3の比率はrun #7/8時点の50%から増加。
- **アラカザム系統: 6戦1勝5敗（16.7%）**——クセロシキ4枚化後もこの小サンプル
  では明確な改善は見られなかった（5敗中4敗がreason=3、1敗がreason=2＝
  長期戦の末に自分がデッキアウト、という既知のパターン通り）。母数が小さい
  ため「効果が無かった」と断定はできないが、少なくとも劇的な改善では
  なかった。
- **新知見**: カードデータ（`before/data/EN_Card_Data.csv`）を確認したところ、
  アラカザム（id 743）は**HP140**、こちらの最大打点（イワパレスのグレート
  シザー、草無無120）では**進化後は理論上一撃で狩れない**ことが判明。
  アブラ（HP50, id 741）・ケーシィ（HP80, id 742）は120ダメージの射程内
  なので、既存の`_boss_priority`/`_select_boss_target`ロジックで進化前に
  狩れる可能性はあるが、進化されてしまうと手が無い。
- その他の敗因（reason=1の5敗）は、ミラー戦（相手も同じクラスタル系統、
  ep `89351128`）、既知の炎弱点（Cinderace/Mega Starmie ex、ep
  `89353827`）、275ステップの長期戦でのマツリーカ系統敗北（ep
  `89354913`）など多様で、単一の新パターンは見つからなかった。

**今回の変更**: run #9のバックログ追記で「次の一手」として挙げていた
「Pokégear 3.0等のサーチ効果をクセロシキの策略へ能動的に誘導する」を実装。
`_select_search_target()`（デッキからのサーチ全般を扱う唯一のハンドラ——
ポケモンサーチ・サポーターサーチ両方がここを通る）に、相手の手札が
`_XEROSIC_HAND_THRESHOLD`（7枚）以上に貯まっている場合はクセロシキの策略
を選択肢の中で最優先にするロジックを追加（該当条件でなければ従来通り
`_BRING_ORDER`ベースの優先度）。ポケモンサーチの選択肢にクセロシキが
出現することは無いため、既存のイシズマイ/ザルードのサーチ優先には影響
しない。`deck.csv`の変更なし（run #9からの継続）。

**テスト**:
- `python -c "import ast; ast.parse(...)"`: 構文OK。
- スモークテスト: `arena.py --p0 agents/iwaparesu_yoshida_v2 --p1 agents/archive/baseline
  --games 40` → **60.0%、errors: 0**。クラッシュ確認のみ。
- `submission/`に`main.py`のみ同期（`deck.csv`はrun #9からの継続で差分なし
  を確認）、standalone importで`main.agent`呼び出し可能・`read_deck_csv()`
  が60枚返すことを確認。

**提出**: ユーザー確認の上、`PYTHONIOENCODING=utf-8`を付けて
`kaggle competitions submit`実行（run #9で無指定実行時にcp932の
UnicodeEncodeErrorが出た教訓を反映——今回はエラー無く成功メッセージが
正常表示された）。ref **`55174301`**、2026-08-02 ~01:05 UTC時点でPENDING。
本日1回目の提出。**次回、アラカザム戦の勝率・reason=3比率の変化を要確認**。
...
## 2026-08-01 (loop run #8, ~23:00-23:30 JST, インタラクティブ実行 / iwaparesu-loop skill)

**注記**: これ以降のログはユーザーの指示により日本語で記載する。

**オリエンテーション**: バックログを確認。トップ項目はrun #7の"次回への申し送り"——
「reason=3バケットの残り5/8は既知の原因に帰着したが、まだ試せていない案として
『クラスタル系統(イシズマイ/イワパレス)を倒された後に戻す冗長性を増やす』が
あるかもしれない」——これを検証するのが今回の主な作業。

**現在の順位**:
- `55157226`（run #7のポフィン修正）は**567.6**（run #7時点の555.5からわずかに上方修正）。
  リーダーボード（最新CSV、6095チーム）: 銅メダルボーダー（上位10%、609位）=**836.1**、
  ほぼ横ばい。現在の順位は**3828位**（ベストスコア576.7）。本日(08-01)の提出は今回開始
  時点で2回（`55151278` 01:50 UTC, `55157226` 07:51 UTC）。

**git push未反映問題（run #7で発生していた件）**: セッション開始時に確認したところ、
run #7の2コミット（`d1c7362`, `3f9a218`）がローカルにのみ残りoriginに未反映だった。
今回のインタラクティブセッションでは対話的な資格情報プロンプトが機能し、
`git push origin main` が問題なく成功した。ヘッドレス実行（タスクスケジューラ経由）
特有の制約という run #7 の推測が正しかったことを裏付ける結果。

**新規リプレイデータはほぼ皆無**: `55157226`の公開エピソード数はrun #7時点の35件から
36件に増えただけ（新規2件のみ、いずれも通常の相手デッキアウト勝ちで新知見なし）。
新規提出もなし。そのため、薄いデータから無理に仮説を立てるのではなく、既存36
エピソード全体を**より精度の高い手法で再解析**する方針に切り替えた。

**手法メモ（今後の実行でも活用推奨）**: リプレイJSONの各ステップの
`steps[0][0]['visualize']`配列に、**両プレイヤーの手札・トラッシュを含む完全非マスクの
生ログ**（各エントリに`current`スナップショットと`logs`イベント配列）が格納されている
ことを発見。従来手法（各エージェント視点の`observation`から推測する方式、片方の視点
しか手札が見えない）より正確・簡潔。エンジン自身が出す`{reason, result,
type:'Result'}`イベントもここに1件だけ、試合終了時に記録される。この手法で
**run #7の記録に誤りを1件発見**：エピソード`89325603`（"イオンのボルトロス自爆技"の
ケース）はreason=3として記録されていたが、実際はreason=1（相手が通常通りサイドを
取り切っただけで、こちらの場が空だったのは同時に起きた結果に過ぎない）だった。
なお`/tmp/...`パスは今回も、Bash組み込みコマンドからは正しく解決されるが、
Windowsネイティブバイナリ（python.exe等）にリテラル文字列として渡すと別の場所に
解決される問題を再確認（run #7と同じ既知の落とし穴）。今回はスクラッチパスを
使い都度argv経由で渡すことで回避した。また`kaggle`コマンドの出力を素のまま
表示するとcp932コーデックエラーで例外を吐く場合がある（アクセント記号付き
カード名など）ため、`PYTHONIOENCODING=utf-8`を付けて実行するのが安全。

**「クラスタル系統の冗長性」を検証**（詳細はバックログのResolvedセクション参照）:
36エピソード全体をこの新手法で再解析した結果、reason=3の8敗のうち5/8は試合中に
該当ライン(イシズマイ/イワパレス)を2枚以上見ていたにもかかわらず敗北しており、
「予備切れ」が原因ではなかった。しかも該当カードは既に各4枚（上限）採用済みで
増枠の余地がない。残り3/8は「ザルード単騎、ダウェブル未ドロー」という既知の
ポフィン残存パターンで、うち2件は同一の攻撃（マツリーカ id 648, attackId 937,
正確に-180ダメージ）でザルードを一撃で倒されていた（ザルードはex無効特性を
持たないため想定通りの弱点）。**結論**: このレバーは有効ではないと判明し
クローズ。新たな有効な改善案は見つからなかった。

**reason=1（通常のサイド負け）6敗の相手デッキも確認**: Hop's Snorlax/Dunsparce系、
Marnie's Grimmsnarl ex系、Dragapult ex系×2、Ethan's Typhlosion（既知の炎弱点）、
Iono's Bellibolt ex系——特定の型への偏りは見られず、多様な相手に対する通常の
接戦負けが中心という印象（深掘りするほどの単一パターンは今回は見つからず）。

**今回の変更**: main.py/deck.csvともに**変更なし**（run #7の`55157226`から
コード・デッキともに同一）。上記の通り根拠のある新しい改善案が見つからなかったため。

**今回の提出**: ユーザーからの明示的な指示により、**コード/デッキ変更の有無に
かかわらず毎回必ず提出する**方針に変更（従来は「根拠の薄い変更で提出枠を
使わない」方針だったが、ユーザーが方針転換を希望）。ref **`55164361`**、
2026-08-01 14:25 UTC、提出時点でPENDING。本日3回目の提出。**次回、このスコア/
リプレイを確認すること**（ただしmain.py/deck.csvは`55157226`と完全同一なので、
差分としての学びは期待できず、単純にスコアのブレ幅・サンプル追加としての
意味合いが大きい）。

**ユーザーからの新方針（重要、今後のrunすべてに適用）**:
1. コード/デッキに変更がなくても、毎回必ずKaggleへ提出すること。
2. 今後、このskill（iwaparesu-loop）のログ・会話上の説明は日本語で行うこと
   （このエントリ以降のIMPROVEMENT_LOG.mdの記載も日本語とする。過去のエントリ
   は英語のまま残す）。

**次回への申し送り**:
1. ref `55164361`のスコアを確認（コード同一のためリプレイ解析よりはスコアの
   ブレ幅確認が主目的）。
2. 「クラスタル系統の冗長性」は検証済み・クローズ。次に掘るならバックログの
   アラカザム対策、または自己参照型ダメージカウンター技・トラッシュ枚数依存
   ダメージ技（未着手、優先度低）。
3. 上記の新方針（毎回提出・日本語ログ）を必ず引き継ぐこと。
